...
#[inline]
unsafe fn otherwhite(g: *mut GlobalState) -> u8 { unsafe {
    (*g).currentwhite ^ WHITEBITS
}}

#[inline]
unsafe fn luaC_white(g: *mut GlobalState) -> u8 { unsafe {
    (*g).currentwhite & WHITEBITS
}}

#[inline]
unsafe fn isgray(o: *mut GCObject) -> bool { unsafe {
...
#[inline]
unsafe fn isdead(g: *mut GlobalState, o: *mut GCObject) -> bool { unsafe {
    isdeadm(otherwhite(g), (*o).marked)
}}

#[inline]
unsafe fn getage(o: *mut GCObject) -> u8 { unsafe {
    (*o).marked & AGEBITS
}}

#[inline]
unsafe fn setage(o: *mut GCObject, age: u8) { unsafe {
    (*o).marked = ((*o).marked & !AGEBITS) | age;
}}

#[inline]
unsafe fn isold(o: *mut GCObject) -> bool { unsafe {
    getage(o) > G_SURVIVAL
}}

#[inline]
unsafe fn makewhite(g: *mut GlobalState, o: *mut GCObject) { unsafe {
    (*o).marked = ((*o).marked & !((1 << BLACKBIT) | WHITEBITS)) | luaC_white(g);
}}

#[inline]
unsafe fn set2gray(o: *mut GCObject) { unsafe {
    (*o).marked &= !((1 << BLACKBIT) | WHITEBITS);
}}

#[inline]
unsafe fn set2black(o: *mut GCObject) { unsafe {
    (*o).marked = ((*o).marked & !WHITEBITS) | bitmask(BLACKBIT);
}}

#[inline]
unsafe fn nw2black(o: *mut GCObject) { unsafe {
    debug_assert!(!iswhite(o));
    (*o).marked |= bitmask(BLACKBIT);
}}

#[inline]
unsafe fn tofinalize(o: *mut GCObject) -> bool { unsafe {
    (*o).marked & bitmask(FINALIZEDBIT) != 0
}}

#[inline]
unsafe fn resetbit(x: &mut u8, b: u8) { unsafe {
    *x &= !bitmask(b);
}}

#[inline]
unsafe fn l_setbit(x: &mut u8, b: u8) { unsafe {
    *x |= bitmask(b);
}}

#[inline]
unsafe fn gcrunning(g: *mut GlobalState) -> bool { unsafe {
    (*g).gcstp == 0
}}

#[inline]
unsafe fn issweepphase(g: *mut GlobalState) -> bool { unsafe {
    GCSswpallgc <= (*g).gcstate && (*g).gcstate <= GCSswpend
}}

#[inline]
unsafe fn keepinvariant(g: *mut GlobalState) -> bool { unsafe {
    (*g).gcstate <= GCSatomic
}}

#[inline]
unsafe fn upisopen(uv: *mut UpVal) -> bool { unsafe {
    !ptr::eq((*uv).v.p, ptr::addr_of_mut!((*uv).u.value))
}}

#[inline]
unsafe fn isintwups(th: *mut lua_State) -> bool { unsafe {
    !ptr::eq((*th).twups, th)
}}
...
#[inline]
unsafe fn sizeudata(nuv: u16, nb: usize) -> usize { unsafe {
    udatamemoffset(nuv) + nb
}}

#[inline]
unsafe fn gnodelast(h: *mut Table) -> *mut Node { unsafe {
    gnode(h, sizenode(h))
}}

#[inline]
unsafe fn gnode(t: *mut Table, i: u32) -> *mut Node { unsafe {
    (*t).node.add(i as usize)
}}

#[inline]
unsafe fn sizenode(t: *mut Table) -> u32 { unsafe {
    1u32 << (*t).lsizenode
}}

#[inline]
unsafe fn gval(n: *mut Node) -> *mut TValue { unsafe {
    ptr::addr_of_mut!((*n).i_val)
}}

#[inline]
unsafe fn keytt(n: *mut Node) -> u8 { unsafe {
    (*n).u.key_tt
}}

#[inline]
unsafe fn keyisnil(n: *mut Node) -> bool { unsafe {
    keytt(n) == LUA_TNIL
}}

#[inline]
unsafe fn keyiscollectable(n: *mut Node) -> bool { unsafe {
    keytt(n) & BIT_ISCOLLECTABLE != 0
}}

#[inline]
unsafe fn gckey(n: *mut Node) -> *mut GCObject { unsafe {
    (*n).u.key_val.gc
}}

#[inline]
unsafe fn gckeyN(n: *mut Node) -> *mut GCObject { unsafe {
...
#[inline]
unsafe fn setdeadkey(n: *mut Node) { unsafe {
    (*n).u.key_tt = 11;
}}

#[inline]
unsafe fn isempty(v: *const TValue) -> bool { unsafe {
    ttisnil(v)
}}

#[inline]
unsafe fn setempty(v: *mut TValue) { unsafe {
    (*v).tt_ = LUA_VEMPTY;
    (*v).value_.gc = ptr::null_mut();
}}

#[inline]
unsafe fn valiswhite(v: *const TValue) -> bool { unsafe {
    iscollectable(v) && iswhite(gcvalue(v))
}}

#[inline]
unsafe fn keyiswhite(n: *mut Node) -> bool { unsafe {
    keyiscollectable(n) && iswhite(gckey(n))
}}

#[inline]
unsafe fn gcvalueN(o: *const TValue) -> *mut GCObject { unsafe {
    if iscollectable(o) {
        gcvalue(o)
    } else {
        ptr::null_mut()
...
#[inline]
unsafe fn gcvalarr(t: *mut Table, i: u32) -> *mut GCObject { unsafe {
...
#[inline]
unsafe fn markvalue(g: *mut GlobalState, o: *mut TValue) { unsafe {
    if valiswhite(o) {
...
#[inline]
unsafe fn markkey(g: *mut GlobalState, n: *mut Node) { unsafe {
    if keyiswhite(n) {
...
#[inline]
unsafe fn markobject<T>(g: *mut GlobalState, t: *mut T) { unsafe {
    let o = obj2gco(t);
    if iswhite(o) {
...
#[inline]
unsafe fn markobjectN<T>(g: *mut GlobalState, t: *mut T) { unsafe {
    if !t.is_null() {
        markobject(g, t);
    }
}}
...
#[inline]
unsafe fn getgclist(o: *mut GCObject) -> *mut *mut GCObject { unsafe {
    match (*o).tt {
...
#[inline]
unsafe fn linkgclist_(o: *mut GCObject, pnext: *mut *mut GCObject, list: *mut *mut GCObject) { unsafe {
...
#[inline]
unsafe fn linkobjgclist(o: *mut GCObject, list: *mut *mut GCObject) { unsafe {
    linkgclist_(o, getgclist(o), list);
}}

#[inline]
unsafe fn linkgclist_table(o: *mut Table, list: *mut *mut GCObject) { unsafe {
    linkgclist_(obj2gco(o), ptr::addr_of_mut!((*o).gclist), list);
}}

#[inline]
unsafe fn linkgclist_udata(o: *mut Udata, list: *mut *mut GCObject) { unsafe {
    linkgclist_(obj2gco(o), ptr::addr_of_mut!((*o).gclist), list);
}}

#[inline]
unsafe fn linkgclist_lclosure(o: *mut LClosure, list: *mut *mut GCObject) { unsafe {
    linkgclist_(obj2gco(o), ptr::addr_of_mut!((*o).gclist), list);
}}

#[inline]
unsafe fn linkgclist_cclosure(o: *mut CClosure, list: *mut *mut GCObject) { unsafe {
    linkgclist_(obj2gco(o), ptr::addr_of_mut!((*o).gclist), list);
}}

#[inline]
unsafe fn linkgclist_thread(o: *mut lua_State, list: *mut *mut GCObject) { unsafe {
    linkgclist_(obj2gco(o), ptr::addr_of_mut!((*o).gclist), list);
}}

#[inline]
unsafe fn linkgclist_proto(o: *mut Proto, list: *mut *mut GCObject) { unsafe {
    linkgclist_(obj2gco(o), ptr::addr_of_mut!((*o).gclist), list);
}}

unsafe fn objsize(o: *mut GCObject) -> l_mem { unsafe {
    match (*o).tt {
...
unsafe fn clearkey(n: *mut Node) { unsafe {
...
unsafe fn iscleared(g: *mut GlobalState, o: *mut GCObject) -> bool { unsafe {
    if o.is_null() {
        false
...
#[unsafe(no_mangle)]
pub unsafe  fn luaC_barrierback_(L: *mut lua_State, o: *mut GCObject) { unsafe {
    let g = G(L);
...
#[unsafe(no_mangle)]
pub unsafe  fn luaC_fix(L: *mut lua_State, o: *mut GCObject) { unsafe {
    let g = G(L);
...
unsafe fn reallymarkobject(g: *mut GlobalState, o: *mut GCObject) { unsafe {
...
unsafe fn markmt(g: *mut GlobalState) { unsafe {
    for i in 0..LUA_NUMTYPES as usize {
        markobjectN(g, (&mut (*g).mt)[i]);
    }
}}

unsafe fn markbeingfnz(g: *mut GlobalState) { unsafe {
    let mut o = (*g).tobefnz;
    while !o.is_null() {
        markobject(g, o);
        o = (*o).next;
...
unsafe fn remarkupvals(g: *mut GlobalState) { unsafe {
...
unsafe fn cleargraylists(g: *mut GlobalState) { unsafe {
    (*g).gray = ptr::null_mut();
...
unsafe fn restartcollection(g: *mut GlobalState) { unsafe {
    cleargraylists(g);
    (*g).gcmarked = 0;
...
unsafe fn genlink(g: *mut GlobalState, o: *mut GCObject) { unsafe {
...
unsafe fn traverseweakvalue(g: *mut GlobalState, h: *mut Table) { unsafe {
...
unsafe fn traversearray(g: *mut GlobalState, h: *mut Table) -> c_int { unsafe {
    let mut marked = 0;
    for i in 0..(*h).asize {
        let o = gcvalarr(h, i);
...
unsafe fn traverseephemeron(g: *mut GlobalState, h: *mut Table, inv: c_int) -> c_int { unsafe {
    let mut hasclears = 0;
    let mut hasww = 0;
...
unsafe fn traversestrongtable(g: *mut GlobalState, h: *mut Table) { unsafe {
    let limit = gnodelast(h);
    traversearray(g, h);
    let mut n = gnode(h, 0);
    while n < limit {
...
unsafe fn gfasttm(g: *mut GlobalState, mt: *mut Table, e: usize) -> *const TValue { unsafe {
    if checknoTM(mt, e) {
        ptr::null()
    } else {
...
unsafe fn getmode(g: *mut GlobalState, h: *mut Table) -> c_int { unsafe {
...
unsafe fn traversetable(g: *mut GlobalState, h: *mut Table) -> l_mem { unsafe {
...
unsafe fn traverseudata(g: *mut GlobalState, u: *mut Udata) -> l_mem { unsafe {
    markobjectN(g, (*u).metatable);
...
unsafe fn traverseproto(g: *mut GlobalState, f: *mut Proto) -> l_mem { unsafe {
...
unsafe fn traverseCclosure(g: *mut GlobalState, cl: *mut CClosure) -> l_mem { unsafe {
...
unsafe fn traverseLclosure(g: *mut GlobalState, cl: *mut LClosure) -> l_mem { unsafe {
    markobjectN(g, (*cl).p);
    for i in 0..(*cl).nupvalues as usize {
...
unsafe fn traversethread(g: *mut GlobalState, th: *mut lua_State) -> l_mem { unsafe {
...
unsafe fn propagatemark(g: *mut GlobalState) -> l_mem { unsafe {
    let o = (*g).gray;
    nw2black(o);
    (*g).gray = *getgclist(o);
    match (*o).tt {
...
unsafe fn propagateall(g: *mut GlobalState) { unsafe {
    while !(*g).gray.is_null() {
        propagatemark(g);
    }
}}

unsafe fn convergeephemerons(g: *mut GlobalState) { unsafe {
    let mut changed;
    let mut dir = 0;
    loop {
...
unsafe fn clearbykeys(g: *mut GlobalState, mut l: *mut GCObject) { unsafe {
    while !l.is_null() {
        let h = gco2t(l);
        let limit = gnodelast(h);
        let mut n = gnode(h, 0);
...
unsafe fn clearbyvalues(g: *mut GlobalState, mut l: *mut GCObject, f: *mut GCObject) { unsafe {
    while l != f {
        let h = gco2t(l);
        for i in 0..(*h).asize {
            let o = gcvalarr(h, i);
...
unsafe fn freeupval(L: *mut lua_State, uv: *mut UpVal) { unsafe {
    if upisopen(uv) {
        luaF_unlinkupval(uv);
    }
...
unsafe fn freeobj(L: *mut lua_State, o: *mut GCObject) { unsafe {
    match (*o).tt {
...
unsafe fn sweeptolive(L: *mut lua_State, mut p: *mut *mut GCObject) -> *mut *mut GCObject { unsafe {
    loop {
        let old = p;
        p = sweeplist(L, p, 1);
        if p != old {
...
unsafe fn checkSizes(L: *mut lua_State, g: *mut GlobalState) { unsafe {
    if (*g).gcemergency == 0 && (*g).strt.nuse < (*g).strt.size / 4 {
        luaS_resize(L, (*g).strt.size / 2);
    }
}}

unsafe fn udata2finalize(g: *mut GlobalState) -> *mut GCObject { unsafe {
    let o = (*g).tobefnz;
    (*g).tobefnz = (*o).next;
    (*o).next = (*g).allgc;
    (*g).allgc = o;
...
unsafe fn dothecall(L: *mut lua_State, _ud: *mut c_void) { unsafe {
    luaD_callnoyield(L, (*L).top.p.sub(2), 0);
}}

unsafe fn setgcovalue(L: *mut lua_State, obj: *mut TValue, gcobj: *mut GCObject) { unsafe {
    (*obj).value_.gc = gcobj;
    (*obj).tt_ = (*gcobj).tt | BIT_ISCOLLECTABLE;
    let _ = L;
}}

unsafe fn GCTM(L: *mut lua_State) { unsafe {
    let g = G(L);
    let mut v = TValue {
        value_: Value {
            gc: ptr::null_mut(),
...
unsafe fn callallpendingfinalizers(L: *mut lua_State) { unsafe {
    while !(*G(L)).tobefnz.is_null() {
        GCTM(L);
    }
}}

unsafe fn findlast(mut p: *mut *mut GCObject) -> *mut *mut GCObject { unsafe {
    while !(*p).is_null() {
...
unsafe fn separatetobefnz(g: *mut GlobalState, all: c_int) { unsafe {
...
unsafe fn checkpointer(p: *mut *mut GCObject, o: *mut GCObject) { unsafe {
    if *p == o {
        *p = (*o).next;
    }
}}

unsafe fn correctpointers(g: *mut GlobalState, o: *mut GCObject) { unsafe {
...
pub(crate) unsafe fn luaC_checkfinalizer(L: *mut lua_State, o: *mut GCObject, mt: *mut Table) { unsafe {
    let g = G(L);
...
unsafe fn setpause(g: *mut GlobalState) { unsafe {
...
unsafe fn sweep2old(L: *mut lua_State, mut p: *mut *mut GCObject) { unsafe {
    let g = G(L);
    while !(*p).is_null() {
        let curr = *p;
        if iswhite(curr) {
...
unsafe fn correctgraylist(mut p: *mut *mut GCObject) -> *mut *mut GCObject { unsafe {
    while !(*p).is_null() {
        let curr = *p;
...
unsafe fn correctgraylists(g: *mut GlobalState) { unsafe {
...
unsafe fn markold(g: *mut GlobalState, from: *mut GCObject, to: *mut GCObject) { unsafe {
    let mut p = from;
    while p != to {
        if getage(p) == G_OLD1 {
            setage(p, G_OLD);
...
unsafe fn finishgencycle(L: *mut lua_State, g: *mut GlobalState) { unsafe {
    correctgraylists(g);
    checkSizes(L, g);
...
unsafe fn minor2inc(L: *mut lua_State, g: *mut GlobalState, kind: u8) { unsafe {
...
unsafe fn checkminormajor(g: *mut GlobalState) -> c_int { unsafe {
...
unsafe fn youngcollection(L: *mut lua_State, g: *mut GlobalState) { unsafe {
    let mut addedold1 = 0;
...
unsafe fn atomic2gen(L: *mut lua_State, g: *mut GlobalState) { unsafe {
    cleargraylists(g);
    (*g).gcstate = GCSswpallgc;
...
unsafe fn setminordebt(g: *mut GlobalState) { unsafe {
    luaE_setdebt(g, luaO_applyparam((*g).gcparams[0], (*g).gcmajorminor));
}}

unsafe fn entergen(L: *mut lua_State, g: *mut GlobalState) { unsafe {
...
pub(crate) unsafe fn luaC_changemode(L: *mut lua_State, newmode: c_int) { unsafe {
    let g = G(L);
...
unsafe fn fullgen(L: *mut lua_State, g: *mut GlobalState) { unsafe {
    minor2inc(L, g, KGC_INC);
    entergen(L, g);
}}

unsafe fn checkmajorminor(L: *mut lua_State, g: *mut GlobalState) -> c_int { unsafe {
    if (*g).gckind == KGC_GENMAJOR {
...
unsafe fn entersweep(L: *mut lua_State) { unsafe {
    let g = G(L);
    (*g).gcstate = GCSswpallgc;
    (*g).sweepgc = sweeptolive(L, ptr::addr_of_mut!((*g).allgc));
}}

unsafe fn deletelist(L: *mut lua_State, mut p: *mut GCObject, limit: *mut GCObject) { unsafe {
    while p != limit {
        let next = (*p).next;
        freeobj(L, p);
        p = next;
...
#[unsafe(no_mangle)]
pub unsafe  fn luaC_freeallobjects(L: *mut lua_State) { unsafe {
    let g = G(L);
    (*g).gcstp = GCSTPCLS;
...
unsafe fn atomic(L: *mut lua_State) { unsafe {
    let g = G(L);
...
unsafe fn singlestep(L: *mut lua_State, fast: c_int) -> l_mem { unsafe {
    let g = G(L);
    (*g).gcstopem = 1;
...
pub(crate) unsafe fn luaC_runtilstate(L: *mut lua_State, state: c_int, fast: c_int) { unsafe {
    let g = G(L);
...
unsafe fn incstep(L: *mut lua_State, g: *mut GlobalState) { unsafe {
...
#[unsafe(no_mangle)]
pub unsafe  fn luaC_step(L: *mut lua_State) { unsafe {
    let g = G(L);
    if !gcrunning(g) {
...
unsafe fn fullinc(L: *mut lua_State, g: *mut GlobalState) { unsafe {
    if keepinvariant(g) {
        entersweep(L);
    }
    luaC_runtilstate(L, GCSpause as c_int, 1);
...
#[unsafe(no_mangle)]
pub unsafe  fn luaC_fullgc(L: *mut lua_State, isemergency: c_int) { unsafe {
    let g = G(L);
...
unsafe fn libc_strchr(s: *const c_char, c: c_int) -> *const c_char { unsafe {
    let mut p = s;
    while !p.is_null() && *p != 0 {
        if *p as c_int == c {
            return p;
