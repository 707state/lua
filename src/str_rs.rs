...
unsafe fn add_value(
    ...
    if unsafe { lua_toboolean(ms.state, -1) } == 0 {
        ...
        let _ = unsafe {
            luaL_error(
                ms.state,
                ERR_INVALID_REPLACEMENT_VALUE_FMT.as_ptr().cast(),
                lua_typename(ms.state, lua_type(ms.state, -1)),
            )
        };
        false
    } else {
    ...
    match opt {
        ...
        _ => {
            let _ = unsafe {
                luaL_error(
                    h.state,
                    ERR_INVALID_FORMAT_OPTION_FMT.as_ptr().cast(),
                    c_int::from(opt),
                )
            };
            KOption::Knop
...
unsafe fn checkformat(state: *mut lua_State, form: &[u8], flags: &[u8], precision: bool) {
    ...
    if spec >= form.len() || !(form[spec] as char).is_ascii_alphabetic() {
        let cform = std::ffi::CString::new(form).unwrap();
        let _ = unsafe {
            luaL_error(
                state,
                ERR_INVALID_CONVERSION_SPEC_FMT.as_ptr().cast(),
                cform.as_ptr(),
            )
        };
    }
...
pub unsafe extern "C" fn str_format(state: *mut lua_State) -> c_int {
    ...
    while i < bytes.len() {
        ...
        match spec {
            ...
            _ => {
                let cform = std::ffi::CString::new(form[..form.len() - 1].to_vec()).unwrap();
                let _ = unsafe {
                    luaL_error(
                        state,
                        ERR_INVALID_CONVERSION_FMT.as_ptr().cast(),
                        cform.as_ptr(),
                    )
                };
            }
        ...
        for i in limit..size {
            if bytes[if islittle { i } else { size - 1 - i }] != mask {
                let _ = unsafe {
                    luaL_error(
                        state,
                        ERR_INT_DOES_NOT_FIT_FMT.as_ptr().cast(),
                        size as c_int,
                    )
                };
            }
