...
mod runtime {
    ...
    const LUA_TDEADKEY: u8 = 11;

    const LUA_VNIL: u8 = 0;
    const LUA_VEMPTY: u8 = 16;
    const LUA_VABSTKEY: u8 = 32;
    ...
    const LUA_VLIGHTUSERDATA: u8 = 2;
    const LUA_VLCF: u8 = 22;
    ...
    const TM_EQ: usize = 5;
    const TM_NEWINDEX: usize = 1;
    const MASKFLAGS: u8 = !(!0u8 << (TM_EQ + 1));
    ...
    const HOK: c_int = 0;
    const HNOTFOUND: c_int = 1;
    const HFIRSTNODE: c_int = 3;
    ...
    unsafe extern "C" {
        ...
        fn luaO_ceillog2(x: u32) -> u8;
        fn luaS_eqstr(a: *mut TString, b: *mut TString) -> c_int;
        fn luaS_hashlongstr(ts: *mut TString) -> u32;
        fn luaS_normstr(state: *mut lua_State, ts: *mut TString) -> *mut TString;
    ...
    #[inline]
    unsafe fn ttypetag(value: *const TValue) -> u8 {
        unsafe { rawtt(value) & 0x3f }
    }

    #[inline]
    unsafe fn ttype(value: *const TValue) -> u8 {
        unsafe { rawtt(value) & 0x0f }
    }

    #[inline]
    unsafe fn ttisnil(value: *const TValue) -> bool {
        unsafe { ttype(value) == LUA_TNIL }
    }
    ...
    #[inline]
    unsafe fn ttislngstring(value: *const TValue) -> bool {
        unsafe { rawtt(value) == ctb(LUA_VLNGSTR) }
    }

    #[inline]
    unsafe fn iscollectable(value: *const TValue) -> bool {
        unsafe { (rawtt(value) & BIT_ISCOLLECTABLE) != 0 }
    }
    ...
    #[inline]
    unsafe fn pvalue(value: *const TValue) -> *mut c_void {
        unsafe { (*value).value_.p }
    }
    ...
    #[inline]
    unsafe fn setsvalue(obj: *mut TValue, ts: *mut TString) {
        unsafe {
            (*obj).value_.gc = ts.cast();
            settt(obj, (*ts).tt | BIT_ISCOLLECTABLE);
        }
    }
    ...
    unsafe fn equalkey(k1: *const TValue, n2: *mut Node, deadok: bool) -> bool {
        if unsafe { rawtt(k1) } != unsafe { keytt(n2) } {
            ...
            match unsafe { keytt(n2) } {
                ...
                x if x == ctb(LUA_VLNGSTR) => unsafe {
                    luaS_eqstr(tsvalue(k1), keystrval(n2)) != 0
                },
        ...
        if hres == HNOTFOUND {
            ...
            if unsafe { ttisnil(key) } {
                ...
                unsafe {
                    ...
                    (*lstate(state)).top.p = top.add(1);
                    luaH_newkey(state, t, s2v(top), value);
                    (*lstate(state)).top.p = top;
                }
                return;
            }
...
pub(crate) unsafe fn raw_luaH_new(state: *mut c_void) -> *mut c_void {
    unsafe { runtime::luaH_new(state.cast()).cast() }
}

pub(crate) unsafe fn raw_luaH_resize(state: *mut c_void, table: *mut c_void, narr: u32, nrec: u32) {
    unsafe { runtime::luaH_resize(state.cast(), table.cast(), narr, nrec) };
}
...
pub(crate) unsafe fn raw_luaH_setint(
    ...
    unsafe { runtime::luaH_setint(state.cast(), table.cast(), key, value.cast()) };
}
...
pub(crate) unsafe fn raw_luaH_getint(
    ...
    unsafe { runtime::luaH_getint(table.cast(), key, result.cast()) }
}
...
pub(crate) unsafe fn raw_luaH_getstr(
    ...
    unsafe { runtime::luaH_getstr(table.cast(), key.cast(), result.cast()) }
}

pub(crate) unsafe fn raw_luaH_Hgetshortstr(table: *mut c_void, key: *mut c_void) -> *const c_void {
    unsafe { runtime::luaH_Hgetshortstr(table.cast(), key.cast()).cast() }
}
...
pub(crate) unsafe fn raw_luaH_getshortstr(
    ...
    unsafe { runtime::luaH_getshortstr(table.cast(), key.cast(), result.cast()) }
}
...
pub(crate) unsafe fn raw_luaH_set(
    ...
    unsafe { runtime::luaH_set(state.cast(), table.cast(), key.cast(), value.cast()) };
}
